...
<div class="green_box">
	...
	<div class="box_content">
    当我们把<code>diffuse</code>和<code>specular</code>传递到字符串流（<code>stringstream</code>）的时候，计数器会增加，在C++自增叫做：变量++，它会先返回自身然后加1，而++变量，先加1再返回自身，我们的例子里，我们先传递原来的计数器值到字符串流，然后再加1，下一轮生效。
	</div>
</div>
