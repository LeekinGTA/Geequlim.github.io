---
layout: post
title: '[第03章 模型] 01 Assimp'
category: 'Learn OpenGL'
tags: 'LearnOpenGL'
previousTitle: '[第02章 光照] 07 复习'
previousUrl: '/2015/07/31/%2007%20Review.html'
nextTitle: '[第03章 模型] 02 Mesh'
nextUrl: '/2015/08/01/%2002%20Mesh.html'
description:
---
...
<img src="http://learnopengl.com/img/model_loading/assimp_structure.png"/>
 ...
 - 一个Mesh会包含多个面片。一个Face（面片）表示渲染中的一个最基本的形状单位，即图元（基本图元有点、线、三角面片、矩形面片）。一个面片记录了一个图元的顶点索引，通过这个索引，可以在mMeshes[]中寻找到对应的顶点位置数据。顶点数据和索引分开存放，可以便于我们使用缓存（VBO、NBO、TBO、IBO）来高速渲染物体。（详见[Hello Triangle](/2015/07/26/%2004%20Hello%20Triangle.html)）
...
你可以到[Assimp的Github](https://github.com/assimp/assimp)去下载Assimp库。到目前为止，Assimp可用的最新版本是3.1.1。我们建议你自己编译Assimp库，因为Assimp官方的已编译库不能很好地覆盖在所有平台上运行。如果你忘记怎样使用CMake编译一个库，请详见[[第01章 入门] 02 创建窗口](/2015/07/26/%2002%20Creating%20a%20window.html)教程。
...
<div class="green_box">
	...
	<div class="box_content">
    如果你想要让Assimp使用多线程支持来提高性能，你可以使用<b>Boost</b>库来编译 Assimp。在<a href="http://assimp.sourceforge.net/lib_html/install.html">Boost安装页面</a>，你能找到关于Boost的完整安装介绍。
	</div>
</div>

现在，你应该已经能够编译Assimp库，并链接Assimp到你的工程里去了。下一节内容：[导入完美的3D物件！](/2015/08/01/%2002%20Mesh.html)
